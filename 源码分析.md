...
```plaintext
+---------------------+
|     API 接口层       | ◄─ JSON-RPC / WebSocket / IPC 交互
+----------+----------+     (eth_sendTransaction, debug_traceTransaction)
           |
+----------v----------+
|  核心功能层          | ◄─ 交易池、区块同步、共识引擎、EVM执行
+----------+----------+     [eth/backend.go的Ethereum结构体整合]
           |
+----------v----------+
|  基础服务层          | ◄─ 网络通信(P2P)、状态存储(LevelDB)、加密算法
+---------------------+